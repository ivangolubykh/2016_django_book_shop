...
def List_Edit_item(request, model, form_edit, sortBy, external_indexes,
                   template_page, template_list, template_editing,
                   template_info):

    ITEMS_TO_PAGE = 3  # количество элементов на странице

    if Veryfy_Admin_Autorizations(request):
        list = model.objects.prefetch_related(*external_indexes).\
            order_by(sortBy)
            ...
            if change_data == 'item_list':
                ...
                if add_form.is_valid():
                    ...
                    return render(request, template_list,
                                  {'form_item_add': form_edit(),
                                   'item_list': list})
                return render(request, template_list,
                              {'form_item_add': add_form, 'item_list': list})
                ...
                return render(request, template_list,
                              {'form_item_add': form_edit, 'item_list': list})

            elif change_data == 'item_start_edit':
                try:
                    edit_id = request.POST['item_id']
                except:
                    return HttpResponse('Ошибка: 006',
                                        content_type='text/html;'
                                                     ' charset=utf-8')
                    # 006 - Ошибка запроса. Нет item_id
                items = get_object_or_404(model, id=edit_id)
                return render(request, template_editing,
                              {'form_item_edit': form_edit(instance=items),
                               'id': edit_id})
                ...
                if form.is_valid():
                    ...
                    return render(request, template_editing,
                                  {'form_item_edit': form, 'id': edit_id})
                ...
                if del_id:
                    ...
                    if user:
                        user.delete()
                        return HttpResponse('<p>Объект удалён успешно.</p>',
                                            content_type='text/html;'
                                                         ' charset=utf-8',
                                            charset='utf-8')
                    else:
                        return HttpResponse('Ошибка: Такого объекта нет в БД.',
                                            content_type='text/html;'
                                                         ' charset=utf-8',
                                            charset='utf-8')
                else:
                    return HttpResponse('Ошибка: 004',
                                        content_type='text/html;'
                                                     ' charset=utf-8',
                                        charset='utf-8')
...
def Admin_Books_Authors(request):
    ...
    external_indexes = ()
    return List_Edit_item(request, Books_Author, Edit_Book_Author, 'baauthor',
                          external_indexes,
                          'admin_book/adminn_books_authors.html',
                          'admin_book/admin_box_books_authors_list.html',
                          'admin_book/admin_box_books_author_editing.html',
...
def Admin_Books_Categories(request):
    # запрос, модель, форма, сортировать по, внешние индексы, шаблон страницы,
        ...
        # шаблон блока одного элемента списка
    external_indexes = ()
    return List_Edit_item(request, Books_Categories, Edit_Book_Categories,
                          'bcname', external_indexes,
                          'admin_book/adminn_books_categor.html',
                          'admin_book/admin_box_books_categor_list.html',
                          'admin_book/admin_box_books_categor_editing.html',
...
def Admin_Books(request):
    ...
    return List_Edit_item(request, Books, Edit_Books, 'bname',
                          external_indexes, 'admin_book/adminn_books.html',
                          'admin_book/admin_box_books_list.html',
                          'admin_book/admin_box_books_editing.html',
